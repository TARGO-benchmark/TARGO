...
class LocalVoxelEncoder_With_InputFushion(nn.Module):
    ...
    def __init__(self, dim=3, input_dim = 1, c_dim=128, unet=False, unet_kwargs=None, unet3d=False, unet3d_kwargs=None, 
                 plane_resolution=512, grid_resolution=None, plane_type='xz', kernel_size=3, padding=0.1):
        super().__init__()
        self.actvn = F.relu
        if kernel_size == 1:
            self.conv_in = nn.Conv3d(input_dim, c_dim, 1)
        else:
            self.conv_in = nn.Conv3d(input_dim, c_dim, kernel_size, padding=1)
    ...
    def forward(self, x):
        ...
        coord1 = coord1.view(1, -1, 1, 1).expand_as(x[:, 0, :, :, :])
        coord2 = coord2.view(1, 1, -1, 1).expand_as(x[:, 0, :, :, :])
        coord3 = coord3.view(1, 1, 1, -1).expand_as(x[:, 0, :, :, :])
...
class LocalVoxelEncoder_without_3d(nn.Module):
    ''' 3D-convolutional encoder network for voxel input.
    ...
    def __init__(self, dim=3, c_dim=128, unet=False, unet_kwargs=None, unet3d=False, unet3d_kwargs=None, in_channels_scale = 3,
                 plane_resolution=512, grid_resolution=None, plane_type='xz', kernel_size=3, padding=0.1, fusion_type='MLP_fusion'):
        super().__init__()
        self.actvn = F.relu
        # if fusion_type == 'transformer_concat':
        if fusion_type in ("transformer_concat", "CNN_concat"):
            # self.c_dim = c_dim * 2
            c_dim = c_dim * 2
        ...
        self.plane_type = plane_type
        self.padding = padding
        # self.conv_ds = nn.Conv1d(in_channels=self.c_dim * in_channels_scale, out_channels=self.c_dim, kernel_size=1, stride=1)
        if fusion_type == 'MLP_fusion':
            self.linear_ds = nn.Linear(in_features=self.c_dim * in_channels_scale, out_features=self.c_dim)
    ...
    def forward(self, x):
        ...
        coord1 = coord1.view(1, -1, 1, 1).expand(batch_size, x.size(2), x.size(3), x.size(4))
        coord2 = coord2.view(1, 1, -1, 1).expand(batch_size, x.size(2), x.size(3), x.size(4))
        coord3 = coord3.view(1, 1, 1, -1).expand(batch_size, x.size(2), x.size(3), x.size(4))
        p = torch.stack([coord1, coord2, coord3], dim=4)
        p = p.view(batch_size, n_voxel, -1)

        # Acquire voxel-wise feature
        c = self.actvn(x)
        c = c.view(batch_size, c.shape[1], -1)
        # c = self.conv_ds(c)
        c = c.permute(0, 2, 1)
        if self.fusion_type == 'MLP_fusion':
            c = self.linear_ds(c)
        # c = c.view(batch_size, self.c_dim, -1)
        # c = self.conv_ds(c)
        # c = c.permute(0, 2, 1)
