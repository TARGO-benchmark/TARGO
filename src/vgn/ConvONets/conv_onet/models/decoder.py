...
class FCDecoder(nn.Module):
    ...
    '''
    def __init__(self, dim=3, c_dim=128, out_dim=1, leaky=False, sample_mode='bilinear', padding=0.1, fusion_type = "MLP_fusion"):
        super().__init__()
        self.c_dim = c_dim
...
class LocalDecoder(nn.Module):
    ...
    def __init__(self, dim=3, c_dim=128,
        ...
        # self.c_dim = c_dim * 2 if fusion_type in ("transformer_concat", "CNN_concat") else c_dim
        c_dim = c_dim * 2 if fusion_type in ("transformer_concat", "CNN_concat") else c_dim
        if concat_feat:
            c_dim *= 3
        self.c_dim = c_dim
        self.n_blocks = n_blocks
        self.no_xyz = no_xyz
        hidden_size = hidden_size * 2 if fusion_type in ("transformer_concat", "CNN_concat") else hidden_size
...
class PatchLocalDecoder(nn.Module):
    ...
    def __init__(self, dim=3, c_dim=128,
                 hidden_size=256, leaky=False, n_blocks=5, sample_mode='bilinear', local_coord=False, pos_encoding='linear', unit_size=0.1, padding=0.1, fusion_type = "MLP_fusion"):
        super().__init__()
        self.c_dim = c_dim
        self.n_blocks = n_blocks
...
class LocalPointDecoder(nn.Module):
    ...
    def __init__(self, dim=3, c_dim=128,
                 hidden_size=256, leaky=False, n_blocks=5, sample_mode='gaussian', fusion_type = "MLP_fusion", **kwargs):
        super().__init__()
        self.c_dim = c_dim
        self.n_blocks = n_blocks
