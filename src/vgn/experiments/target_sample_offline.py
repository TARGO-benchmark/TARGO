...
def run(
    ...
    for num_id, curr_mesh_pose_list in enumerate(os.listdir(test_mesh_pose_list)):
        # if num_id < 8500:
        #     continue
        occ_targ = np.load(test_root + 'scenes/' + curr_mesh_pose_list)['occ_targ']
        # if not (occ_targ >= 0.5 and occ_targ < 0.6):
        #     continue
    # for num_id, curr_mesh_pose_list in enumerate(os.listdir(test_scenes)):
        ...
        curr_scene_path = f'{test_root}scenes/{curr_mesh_pose_list}'
        if visualize:
            visual_dict = {'mesh_name':scene_name, 'mesh_dir':logger.mesh_dir}
        mesh_pos = np.load(test_mesh_pose_list + curr_mesh_pose_list, allow_pickle=True)['pc']
        sim.world.reset()
        ...
        occluder_count = len(mesh_pos.item().keys()) - 1
        ...
        for id, mesh in enumerate(mesh_pos.item().values()):
            ...
            # trimesh_mesh = trimesh.load_mesh(mesh[0])
            # trimesh_mesh.apply_scale(mesh[1])
            # trimesh_mesh.apply_transform(mesh[2])
            ...
            if id != 0:
                ...
                trimesh_mesh.apply_scale(mesh[1])
                trimesh_mesh.apply_transform(mesh[2])
                z_coords = trimesh_mesh.vertices[:, 2]

                # 找到 z 坐标的最大值
                z_max = z_coords.max()
                if id != tgt_id:
                    occluder_height.append(z_max)
                elif id == tgt_id:
                    tgt_height = z_max
                    # 计算每个维度的最小值和最大值
                    min_coords = trimesh_mesh.vertices.min(axis=0)
                    max_coords = trimesh_mesh.vertices.max(axis=0)
        ...
        if len(occluder_height) == 0:
            relative_height = tgt_height
        else:
            relative_height = tgt_height - np.max(occluder_height)
        ...
        if type in ('vgn', 'giga_aff', 'giga', 'giga_hr'):
            ...
            tsdf, timings["integration"], scene_grid, tgt_grid, tgt_mask_vol, occ_level = sim.acquire_single_tsdf_target_grid(curr_scene_path, tgt_id, res, type, complete_shape = complete_shape, curr_mesh_pose_list = scene_name, input_points=input_points, shape_completion= shape_completion)
            ...
            if fusion_type in ('CNN_concat','CNN_add', 'MLP_fusion'):
                tsdf, timings["integration"], scene_grid, tgt_grid, tgt_mask_vol, occ_level = sim.acquire_single_tsdf_target_grid(curr_scene_path, tgt_id, 40, type, fusion_type = fusion_type, complete_shape = complete_shape, curr_mesh_pose_list = scene_name, input_points=input_points, shape_completion= shape_completion)
                state = argparse.Namespace(tsdf=tsdf, tgt_grid = tgt_grid,tgt_mask_vol=tgt_mask_vol, scene_grid= scene_grid,occ_level=occ_level,type = type, fusion_type = fusion_type)
            elif fusion_type in ('transformer_query_scene','transformer_query_target', 'transformer_concat'):
                if shape_completion:
                    tsdf, timings["integration"], scene_no_targ_pc, targ_pc, occ_level = sim.acquire_single_tsdf_target_grid(curr_scene_path, tgt_id, 40, type, fusion_type = fusion_type, complete_shape=complete_shape, curr_mesh_pose_list = scene_name, input_points=input_points, shape_completion= shape_completion)
                    ...
                    tsdf, timings["integration"], scene_pc, targ_pc, tgt_grid, occ_level = sim.acquire_single_tsdf_target_grid(curr_scene_path, tgt_id, 40, type, fusion_type = fusion_type, complete_shape=complete_shape, curr_mesh_pose_list = scene_name, input_points=input_points, shape_completion= shape_completion)
        ...
        ## key value是: occ_level, occluder_count
